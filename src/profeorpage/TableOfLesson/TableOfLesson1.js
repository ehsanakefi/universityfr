      ...
      this.props.persons.persons.map((profesor) =>
        profesor.Lesson.map((lesson, index) =>
          this.props.ADDLessonTable(
            {
              name: profesor.name + " " + profesor.familyName,
              Lesson: lesson,
              days: profesor.day
            }
          )
  ...
  render() {
    return (
      <div className="mainTableOfLesson">
        <div style={{ borderLeft: "4px solid #2B2F3B", display: "flex", width: "45px" }}>
          <Menu />
        </div>
          ...
          <div className="down_TOL">
            <div className="leftProfesor_TOL">
              <div className="centerRight_profesor_TOL" >
                <div className="upDetaliHistory_TOL">
                  <div className="upNowDays_TOL"><p className="textNowDays_TOL">امروز</p></div>
                  <div className="downHitory_TOL">
                    <div className="downHitoryNumDay_TOL"><p className="textNumDay_TOL">24</p></div>
                    <div className="downHitoryNameDay_TOL">
                      <p className="nameDay_TOL">سه شنبه</p>
                      <p className="nameMonth_TOL">اردیبهشت 1398</p>
                    </div>
                  </div>
                </div>

                {this.props.initionalData.prfoesorsLesson.map((data, index) => <div key={index}>{data.name}</div>)}
                {/* {
                ...
                <table>
                  <tbody>
                    {this.state.days.map((days, index) => {
                      return (
                        <tr key={index}>
                          <td style={{ border: "none" }} colSpan="4">{days}</td>

                          {index == 0 ? this.state.time.map((times, index) => {
                            return (

                              <td style={{ border: "none" }} key={index} colSpan="4"> {times.split(' ')[0]}
                                <br />
                                {times.split(' ')[1]}</td>
